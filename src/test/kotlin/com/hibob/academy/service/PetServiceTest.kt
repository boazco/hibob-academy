...
class PetServiceTest {
    ...
    fun `create new pet`() {
        val pet = Pet(UUID.randomUUID(), "jerry", "Dog", 9, Date.valueOf(LocalDate.now()), null)
        whenever(petDaoMock.createPet(pet.name, pet.type, pet.companyId, pet.dateOfArrival, pet.ownersId)).thenReturn(
            pet.id
        )
        assertEquals(pet.id, petDaoMock.createPet(pet.name, pet.type, pet.companyId, pet.dateOfArrival, pet.ownersId))
    }
